...
void KEY_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //设置成上拉输入
 	GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化GPIOC10
 
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_0;//PA0
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PA0设置成输入，默认下拉	  
	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOA.0
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_12;//PC13
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //设置成上拉输入
...
{	 
	...
	if(key_up&&(KEY4==0||KEY1==0||WK_UP==1)||(KEY2==0)||KEY3==0)
	{
		delay_ms(10);//去抖动 
		key_up=0;
	...
	}else if(KEY4==1&&KEY1==1&&WK_UP==0&&KEY2==0&&KEY3==0)key_up=1; 	     
	return 0;// 无按键按下
}
